...
This version writes `/env.js` at container start so Railway variables are read at runtime, not only during Vite build. Keep `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` in Railway Variables.
