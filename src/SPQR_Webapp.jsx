  ...
  return(
    <div className="spqr-senate-scroll" style={{position:"relative",userSelect:"none",overflowX:"auto",paddingBottom:"0.5rem"}}>
      <div style={{position:"relative",width:W,height:H,margin:"0 auto",minWidth:W,background:`radial-gradient(ellipse at 50% 86%, ${T.surf} 0%, ${T.bg} 46%, ${T.card} 100%)`,border:`2px solid ${T.bhi}`,borderRadius:"18px 18px 90px 90px",overflow:"hidden"}}>
        ...
        {activeMotion&&(()=>{const proposer=players.find(p=>p.id===activeMotion.byId);return <button onClick={()=>onMotionClick&&onMotionClick(activeMotion)} style={{position:"absolute",left:cx-92,top:H-330,width:184,minHeight:78,border:`2px solid ${T.gold}`,background:"#fff8ec",boxShadow:"0 4px 18px rgba(0,0,0,0.18)",fontFamily:"'Cinzel',serif",color:T.ghi,cursor:"pointer",padding:"0.45rem",zIndex:4}}>
          <div style={{fontSize:"0.62rem",letterSpacing:"0.12em",color:T.rhi}}>MOTION BEFORE THE SENATE</div>
          <div style={{fontSize:"0.72rem",fontWeight:900,marginTop:"0.18rem"}}>{activeMotion.title}</div>
          <div style={{fontSize:"0.62rem",color:T.mut,marginTop:"0.18rem"}}>Proposed by {proposer?.latinName||activeMotion.byName}</div>
          <div style={{fontSize:"0.58rem",color:T.blue,marginTop:"0.18rem"}}>Click to vote</div>
        </button>})()}
