...
const DEF_GAME={session:1,sessionInSeason:1,year:218,season:"Winter",
  ...
  fgold:55,ffood:25,fpop:200,fturns:2,privateTaxGoldPct:10,privateTaxFoodPct:5,salaryOverrides:{}};
...
const DEF_BUSINESSES=[
  {id:"olive_farm",emoji:"🫒",name:"Olive Estate",costGold:260,costFood:35,incomeGold:24,incomeFood:8,regionCaps:{latium:2,etruria:3,campania:3,apulia:2,calabria:2,sicilia:3}},
  {id:"grain_estate",emoji:"🌾",name:"Grain Estate",costGold:220,costFood:25,incomeGold:12,incomeFood:38,regionCaps:{latium:2,campania:4,apulia:4,sicilia:5,sardinia_corsica:3}},
  {id:"fishery",emoji:"🐟",name:"Fishery",costGold:180,costFood:20,incomeGold:16,incomeFood:28,regionCaps:{picenum:2,calabria:3,sicilia:3,sardinia_corsica:4}},
  {id:"merchant_house",emoji:"🏺",name:"Merchant House",costGold:360,costFood:45,incomeGold:45,incomeFood:0,regionCaps:{latium:3,campania:3,calabria:2,sicilia:3,sardinia_corsica:2}},
  {id:"mine_contract",emoji:"⛏️",name:"Mining Contract",costGold:440,costFood:55,incomeGold:60,incomeFood:0,regionCaps:{etruria:2,samnium:2,gallia_cisalpina:2,illyria:1}},
  {id:"pasture",emoji:"🐑",name:"Pasture Lands",costGold:200,costFood:30,incomeGold:18,incomeFood:24,regionCaps:{samnium:3,lucania:3,bruttium:3,sardinia_corsica:3}},
];
const BALANCED_BUSINESSES=[
  {id:"olive_farm",emoji:"🫒",name:"Olive Estate",costGold:260,costFood:35,incomeGold:14,incomeFood:4,regionCaps:{latium:2,etruria:3,campania:3,apulia:2,calabria:2,sicilia:3}},
  {id:"grain_estate",emoji:"🌾",name:"Grain Estate",costGold:220,costFood:25,incomeGold:6,incomeFood:20,regionCaps:{latium:2,campania:4,apulia:4,sicilia:5,sardinia_corsica:3}},
  {id:"fishery",emoji:"🐟",name:"Fishery",costGold:180,costFood:20,incomeGold:8,incomeFood:14,regionCaps:{picenum:2,calabria:3,sicilia:3,sardinia_corsica:4}},
  {id:"merchant_house",emoji:"🏺",name:"Merchant House",costGold:360,costFood:45,incomeGold:25,incomeFood:0,regionCaps:{latium:3,campania:3,calabria:2,sicilia:3,sardinia_corsica:2}},
  {id:"mine_contract",emoji:"⛏️",name:"Mining Contract",costGold:440,costFood:55,incomeGold:32,incomeFood:0,regionCaps:{etruria:2,samnium:2,gallia_cisalpina:2,illyria:1}},
  {id:"pasture",emoji:"🐑",name:"Pasture Lands",costGold:200,costFood:30,incomeGold:9,incomeFood:12,regionCaps:{samnium:3,lucania:3,bruttium:3,sardinia_corsica:3}},
];
const DEF_WEALTH={gold:160,food:80,householdGold:25,householdFood:18};
const foodSaleRate=0.5;
const POS_SALARIES={
  consul_1:{gold:55,food:20},consul_2:{gold:55,food:20},
  praetor_1:{gold:38,food:15},praetor_2:{gold:38,food:15},
  quaestor_1:{gold:32,food:12},quaestor_2:{gold:32,food:12},
  aedile_1:{gold:28,food:14},tribune_1:{gold:24,food:10},tribune_2:{gold:24,food:10},
  dictator_1:{gold:70,food:25},magister_equitum_1:{gold:42,food:15},praefectus_classis_1:{gold:38,food:14}
};
const salaryFor=(role,game={})=>{
  const base=POS_SALARIES[role]||{gold:0,food:0};
  const ov=game?.salaryOverrides?.[role]||{};
  return {gold:Number(ov.gold??base.gold??0),food:Number(ov.food??base.food??0)};
};
...
const personalBalanceFor=(userId,role,assets,businesses,wealth,game={})=>{
  const gross=personalIncomeFor(userId,assets,businesses);
  ...
  const sal=salaryFor(role,game);
  const w=wealthOf(wealth,userId);
  const houseGold=Number(w.householdGold||0);
  const houseFood=Number(w.householdFood||0);
  return {gross,tax,taxGold,taxFood,salary:sal,houseGold,houseFood,netGold:gross.gold-taxGold+sal.gold-houseGold,netFood:gross.food-taxFood+sal.food-houseFood};
};
...
function PersonalWealthPanel({user,D,onRefresh}){
  ...
  const load=useCallback(async()=>{setBusinesses((await db.get("spqr_biz"))||DEF_BUSINESSES);setAssets((await db.get("spqr_assets"))||[]);setWealth((await db.get("spqr_wealth"))||{});setDonations((await db.get("spqr_donations"))||[]);},[]);
  useEffect(()=>{load();},[load]);
  ...
  return <div>
    ...
    <Card><STit c="Personal Wealth Balance" sub="Exact coin and food are private. Only you and the Game Master can see them."/>
      <div className="spqr-stat-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:"0.55rem"}}>
        <Stat label="🪙 Private Gold" value={`${fmt(myW.gold)}T`} color={RES.gold.color}/><Stat label="🌾 Private Food" value={`${fmt(myW.food)}M`} color={RES.food.color}/><Stat label="Gross Estate Income" value={`🪙 +${fmt(bal.gross.gold)}T / 🌾 +${fmt(bal.gross.food)}M`} color={T.gre}/><Stat label="Taxes" value={`-${fmt(bal.taxGold)}T / -${fmt(bal.taxFood)}M`} color={T.rhi}/><Stat label="Office Salary" value={`+${fmt(bal.salary.gold)}T / +${fmt(bal.salary.food)}M`} color={T.blue}/><Stat label="Household Upkeep" value={`-${fmt(bal.houseGold)}T / -${fmt(bal.houseFood)}M`} color={T.rhi}/><Stat label="Net per Season" value={`${bal.netGold>=0?"+":""}${fmt(bal.netGold)}T / ${bal.netFood>=0?"+":""}${fmt(bal.netFood)}M`} color={(bal.netGold+bal.netFood)>=0?T.gre:T.rhi}/>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:"0.75rem"}}>
        <div style={{border:`1px solid ${RES.gold.color}66`,background:"#FFF8E8",padding:"0.75rem"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"0.35rem",fontSize:"1rem"}}>
            ...
            <span>Office salary</span><b style={{color:T.blue}}>+{fmt(bal.salary.gold)}T</b>
        ...
        <div style={{border:`1px solid ${RES.food.color}66`,background:"#F4FFF0",padding:"0.75rem"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"0.35rem",fontSize:"1rem"}}>
            ...
            <span>Office salary</span><b style={{color:T.blue}}>+{fmt(bal.salary.food)}M</b>
    ...
    <Card><STit c="My Properties by Region"/>{myAssets.length===0?<div style={{color:T.mut,fontStyle:"italic"}}>You own no properties yet.</div>:Object.entries(byRegion).map(([regionId,list])=>{const r=getRegion(regions,regionId);const sum=list.reduce((a,x)=>{const biz=getBiz(businesses,x.typeId);a.g+=Number(biz.incomeGold||0);a.f+=Number(biz.incomeFood||0);return a;},{g:0,f:0});return <Card key={regionId} style={{borderLeft:`4px solid ${T.gold}`}}><STit c={r.name} sub={`Capital: ${r.capital||"Unknown"} · Gross income: 🪙 +${sum.g}T / 🌾 +${sum.f}M`}/>{list.map(a=>{const biz=getBiz(businesses,a.typeId);return <div key={a.id} style={{display:"grid",gridTemplateColumns:"minmax(220px,1fr) auto",gap:"0.5rem",alignItems:"center",padding:"0.55rem",border:`1px solid ${T.border}`,marginBottom:"0.45rem",background:T.bg}}><div><b>{biz.emoji} {biz.name}</b><div style={{color:T.mut}}>Income: 🪙 +{biz.incomeGold}T / 🌾 +{biz.incomeFood}M</div><Row gap="0.35rem" wrap><select value={propTransfer[a.id]||""} onChange={e=>setPropTransfer(x=>({...x,[a.id]:e.target.value}))} style={{padding:"0.32rem",border:`1px solid ${T.border}`,background:T.card}}><option value="">Transfer property to...</option>{otherPlayers.map(p=><option key={p.id} value={p.id}>{p.latinName}</option>)}</select><Btn v="dark" sm onClick={()=>sendProperty(a)}>Transfer</Btn></Row></div><Btn v="red" sm onClick={()=>sellAsset(a)}>Sell</Btn></div>})}</Card>})}</Card>
...
function ABusinesses({D,onRefresh}){
  ...
  useEffect(()=>{(async()=>{setBusinesses((await db.get("spqr_biz"))||DEF_BUSINESSES);setAssets((await db.get("spqr_assets"))||[]);setWealth((await db.get("spqr_wealth"))||{});setDonations((await db.get("spqr_donations"))||[]);setGame({...DEF_GAME,...((await db.get("spqr_g"))||D.game||{})});})();},[D.game]);
  ...
  const setSalary=(role,k,v)=>{const next={...game,salaryOverrides:{...(game.salaryOverrides||{}),[role]:{...((game.salaryOverrides||{})[role]||{}),[k]:Number(v)||0}}};setGame(next);};
  return <div>
    {msg&&<div style={{padding:"0.55rem 0.8rem",background:"#F4FFF0",border:`1px solid ${T.gre}`,color:T.gre,marginBottom:"0.7rem"}}>{msg}</div>}
    ...
    <Card><STit c="Taxes and Magistrate Salaries" sub="Quaestor actions can justify changing taxes. These are applied when the season advances."/><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:"0.6rem"}}><Inp label="🪙 Estate Gold Tax %" type="number" value={game.privateTaxGoldPct??10} onChange={v=>setGame({...game,privateTaxGoldPct:Number(v)||0})}/><Inp label="🌾 Estate Food Tax %" type="number" value={game.privateTaxFoodPct??5} onChange={v=>setGame({...game,privateTaxFoodPct:Number(v)||0})}/></div><STit c="Office Salaries"/>{roleEntries().map(({key,title,emoji})=>{const sal=salaryFor(key,game);return <div key={key} style={{display:"grid",gridTemplateColumns:"minmax(220px,1.5fr) repeat(2,minmax(120px,1fr))",gap:"0.5rem",alignItems:"center",padding:"0.35rem 0",borderBottom:`1px solid ${T.border}`}}><b>{emoji} {title}</b><Inp label="🪙 Gold Salary" type="number" value={sal.gold} onChange={v=>setSalary(key,"gold",v)}/><Inp label="🌾 Food Salary" type="number" value={sal.food} onChange={v=>setSalary(key,"food",v)}/></div>})}</Card>
    <Card><STit c="Senator Balance Sheet" sub="GM-only overview. Exact private wealth is hidden from other players. Edit wealth and household upkeep here."/><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:"0.92rem",minWidth:"980px"}}><thead><tr style={{background:T.bg,color:T.mut,fontFamily:"'Cinzel',serif"}}>{["Senator","🪙 Gold","🌾 Food","Gross Estate","Tax","Salary","Household","Net / Season","Properties"].map(h=><th key={h} style={{textAlign:"left",padding:"0.45rem",border:`1px solid ${T.border}`}}>{h}</th>)}</tr></thead><tbody>{players.map(p=>{const w=wealthOf(wealth,p.id);const bal=personalBalanceFor(p.id,p.role,assets,businesses,wealth,game);return <tr key={p.id}><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,fontWeight:900}}>{p.latinName}</td><td style={{padding:"0.35rem",border:`1px solid ${T.border}`}}><input type="number" value={w.gold} onChange={e=>setPlayerWealth(p.id,"gold",e.target.value)} style={{width:"110px",padding:"0.28rem",color:RES.gold.color}}/></td><td style={{padding:"0.35rem",border:`1px solid ${T.border}`}}><input type="number" value={w.food} onChange={e=>setPlayerWealth(p.id,"food",e.target.value)} style={{width:"110px",padding:"0.28rem",color:RES.food.color}}/></td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,color:T.gre}}>+{bal.gross.gold}T / +{bal.gross.food}M</td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,color:T.rhi}}>-{bal.taxGold}T / -{bal.taxFood}M</td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,color:T.blue}}>+{bal.salary.gold}T / +{bal.salary.food}M</td><td style={{padding:"0.35rem",border:`1px solid ${T.border}`}}><input title="Household Gold" type="number" value={w.householdGold} onChange={e=>setPlayerWealth(p.id,"householdGold",e.target.value)} style={{width:"72px",padding:"0.25rem"}}/> / <input title="Household Food" type="number" value={w.householdFood} onChange={e=>setPlayerWealth(p.id,"householdFood",e.target.value)} style={{width:"72px",padding:"0.25rem"}}/></td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,color:(bal.netGold+bal.netFood)>=0?T.gre:T.rhi}}>{bal.netGold>=0?"+":""}{bal.netGold}T / {bal.netFood>=0?"+":""}{bal.netFood}M</td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,textAlign:"center"}}>{assets.filter(a=>a.ownerId===p.id).length}</td></tr>})}</tbody></table></div></Card>
...
function AResources({D,onRefresh}){
  ...
  const doAdvance=async()=>{
    ...
    for(const pl of players){
      const bal=personalBalanceFor(pl.id,pl.role,assets,businesses,nextWealth,g);
      const w=wealthOf(nextWealth,pl.id);
      totalTaxGold+=bal.taxGold;
      totalTaxFood+=bal.taxFood;
      nextWealth[pl.id]={...w,gold:Math.max(0,Number(w.gold||0)+bal.netGold),food:Math.max(0,Number(w.food||0)+bal.netFood)};
      if(bal.gross.gold||bal.gross.food||bal.salary.gold||bal.salary.food||bal.houseGold||bal.houseFood){
        await addHistory(pl.id,"Private Wealth Balance",`Gross estates: ${bal.gross.gold}T/${bal.gross.food}M. Taxes: ${bal.taxGold}T/${bal.taxFood}M. Office salary: ${bal.salary.gold}T/${bal.salary.food}M. Household upkeep: ${bal.houseGold}T/${bal.houseFood}M. Net: ${bal.netGold}T/${bal.netFood}M for ${sLab(ng)}.`,"wealth");
      }
    }
