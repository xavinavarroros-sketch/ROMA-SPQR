...
function PersonalWealthPanel({user,D,onRefresh}){
  ...
  const load=useCallback(async()=>{setBusinesses((await db.get("spqr_biz"))||DEF_BUSINESSES);setAssets((await db.get("spqr_assets"))||[]);setWealth((await db.get("spqr_wealth"))||{});setDonations((await db.get("spqr_donations"))||[]);setWealthlog((await db.get("spqr_wealthlog"))||[]);},[]);
  useEffect(()=>{load();},[load]);
...
function PlayerApp({user:initUser,onLogout}){
  ...
  const toneColor=tone=>tone==="personal"?"#2563EB":tone==="records"?"#5B21B6":tone==="office"?(pos?.color||T.gold):T.gold;
  const toneBg=tone=>tone==="personal"?"#EFF6FF":tone==="records"?"#F5F0FF":tone==="office"?(pos?.bg||T.surf):"#FFF7E6";
    ...
    <div style={{minHeight:"100vh",background:T.bg}}>
      ...
      <div className="spqr-tabs" style={{display:"flex",borderBottom:`1px solid ${T.border}`,background:T.surf,overflowX:"auto",position:"sticky",top:"45px",zIndex:99}}>
        {TABS.map((it,i)=>it.sep?(
          <div key={`sep-${it.sep}-${i}`} style={{padding:"0.58rem 0.55rem",background:toneBg(it.tone),color:toneColor(it.tone),borderRight:`1px solid ${T.border}`,fontFamily:"'Cinzel',serif",fontSize:"0.62rem",fontWeight:900,letterSpacing:"0.16em",whiteSpace:"nowrap",flexShrink:0,display:"flex",alignItems:"center"}}>{it.sep}</div>
        ):(
...
function AdminApp({onLogout}){
  ...
  const TABS=[
    ...
    {k:"parties",l:"Parties"},
    {k:"cemetery",l:`Cemetery (${(D.cemetery||[]).length})`},
