...
const DEF_BUSINESSES=[
  {id:"olive_farm",emoji:"🫒",name:"Olive Estate",costGold:220,costFood:40,incomeGold:45,incomeFood:15,regionCaps:{latium:2,etruria:3,campania:3,apulia:2,calabria:2,sicilia:3}},
  {id:"grain_estate",emoji:"🌾",name:"Grain Estate",costGold:180,costFood:20,incomeGold:25,incomeFood:65,regionCaps:{latium:2,campania:4,apulia:4,sicilia:5,sardinia_corsica:3}},
  {id:"fishery",emoji:"🐟",name:"Fishery",costGold:150,costFood:20,incomeGold:30,incomeFood:45,regionCaps:{picenum:2,calabria:3,sicilia:3,sardinia_corsica:4}},
  {id:"merchant_house",emoji:"🏺",name:"Merchant House",costGold:300,costFood:50,incomeGold:85,incomeFood:0,regionCaps:{latium:3,campania:3,calabria:2,sicilia:3,sardinia_corsica:2}},
  {id:"mine_contract",emoji:"⛏️",name:"Mining Contract",costGold:360,costFood:60,incomeGold:105,incomeFood:0,regionCaps:{etruria:2,samnium:2,gallia_cisalpina:2,illyria:1}},
  {id:"pasture",emoji:"🐑",name:"Pasture Lands",costGold:170,costFood:30,incomeGold:35,incomeFood:35,regionCaps:{samnium:3,lucania:3,bruttium:3,sardinia_corsica:3}},
];
const DEF_WEALTH={gold:300,food:120,householdGold:50,householdFood:30};
const foodSaleRate=0.5;
const POS_SALARIES={
  consul_1:{gold:140,food:60},consul_2:{gold:140,food:60},
  praetor_1:{gold:95,food:40},praetor_2:{gold:95,food:40},
  quaestor_1:{gold:80,food:35},quaestor_2:{gold:80,food:35},
  aedile_1:{gold:75,food:35},tribune_1:{gold:65,food:30},tribune_2:{gold:65,food:30},
  dictator_1:{gold:180,food:80},magister_equitum_1:{gold:110,food:50},praefectus_classis_1:{gold:100,food:45}
};
const salaryFor=role=>POS_SALARIES[role]||{gold:0,food:0};
...
const personalIncomeFor=(userId,assets,businesses)=> (assets||[]).filter(a=>a.ownerId===userId).reduce((acc,a)=>{const b=getBiz(businesses,a.typeId);acc.gold+=Number(b.incomeGold||0);acc.food+=Number(b.incomeFood||0);return acc;},{gold:0,food:0});
...
function SenatorProfileModal({player,onClose}){
  ...
  const salary=salaryFor(player.role);
      ...
      <div style={{display:"flex",gap:"1rem",alignItems:"flex-start",flexWrap:"wrap"}}>
        ...
        <div style={{flex:1,minWidth:220}}>
          ...
          <div style={{marginTop:"0.7rem",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:"0.45rem"}}>
            ...
            <Stat label="Office Salary" value={player.role?`+${fmt(salary.gold)}T / +${fmt(salary.food)}M`:"No office"} color={T.blue}/>
...
function CharacterPanel({user,onUpdate}){
  ...
  const saveIdentity=async()=>{
    ...
    const updatedUser={...user,latinName,username,charClass:identity.charClass||"Patrician",discord,bio:identity.bio||""};
      ...
      <Card style={{borderLeft:`3px solid ${pos?pos.color:T.gold}`}}>
        ...
        <div style={{borderTop:`1px solid ${T.border}`,paddingTop:"0.8rem",marginTop:"0.8rem"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:"0.6rem"}}>
            ...
            <div>
              <Lbl c="Class"/>
              <select value={identity.charClass} onChange={e=>setIdentity(f=>({...f,charClass:e.target.value}))} style={{width:"100%",background:T.card,border:`1px solid ${T.border}`,color:T.text,padding:"0.48rem 0.6rem",fontFamily:"'EB Garamond',serif",fontSize:"1.05rem"}}>
                {['Patrician','Plebeian','Equestrian','New Man','Allied Noble'].map(x=><option key={x}>{x}</option>)}
              </select>
            </div>
...
function PersonalWealthPanel({user,D,onRefresh}){
  ...
  const load=useCallback(async()=>{setBusinesses((await db.get("spqr_biz"))||DEF_BUSINESSES);setAssets((await db.get("spqr_assets"))||[]);setWealth((await db.get("spqr_wealth"))||{});setDonations((await db.get("spqr_donations"))||[]);setParties((await db.get("spqr_parties"))||[]);},[]);
  useEffect(()=>{load();},[load]);
  const myW=wealthOf(wealth,user.id);
  const myAssets=assets.filter(a=>a.ownerId===user.id);
  const myInc=personalIncomeFor(user.id,assets,businesses);
  const salary=salaryFor(user.role);
  const otherPlayers=(D.players||[]).filter(p=>p.id!==user.id);
  const b=getBiz(businesses,buy.typeId);
  const reg=getRegion(regions,buy.regionId);
  const cap=Number((b.regionCaps||{})[buy.regionId]||0);
  const used=ownedSlots(assets,b.id,buy.regionId);
  const saveWealth=async(next)=>{await db.set("spqr_wealth",next);setWealth(next);};
  const buyAsset=async()=>{
    const biz=getBiz(businesses,buy.typeId);const region=getRegion(regions,buy.regionId);const max=Number((biz.regionCaps||{})[buy.regionId]||0);
    if(max<=0){setMsg("This business is not available in that province.");return;}
    if(ownedSlots(assets,biz.id,buy.regionId)>=max){setMsg("No available slots remain for that business in this province.");return;}
    const w=wealthOf(wealth,user.id);
    if(w.gold<Number(biz.costGold||0)||w.food<Number(biz.costFood||0)){setMsg("Insufficient personal wealth to buy this property.");return;}
    const newAsset={id:Date.now().toString()+Math.random().toString(36).slice(2),ownerId:user.id,ownerName:user.latinName,typeId:biz.id,regionId:region.id,regionName:region.name,boughtAt:sLab(D.game||DEF_GAME),ts:Date.now()};
    const nextAssets=[...assets,newAsset];
    const nextWealth={...wealth,[user.id]:{gold:w.gold-Number(biz.costGold||0),food:w.food-Number(biz.costFood||0)}};
    await db.set("spqr_assets",nextAssets);await saveWealth(nextWealth);await addHistory(user.id,`Bought ${biz.name}`,`${user.latinName} bought ${biz.name} in ${region.name}.`,`estate`);
    setAssets(nextAssets);setMsg("Property purchased.");onRefresh&&onRefresh();setTimeout(()=>setMsg(""),3000);
  };
  const sellAsset=async(asset)=>{
    const biz=getBiz(businesses,asset.typeId);if(!confirm(`Sell ${biz.name} in ${asset.regionName||asset.regionId}? You receive 60% of the original cost.`))return;
    const w=wealthOf(wealth,user.id);const refundG=Math.floor(Number(biz.costGold||0)*0.6);const refundF=Math.floor(Number(biz.costFood||0)*0.6);
    const nextAssets=assets.filter(a=>a.id!==asset.id);const nextWealth={...wealth,[user.id]:{gold:w.gold+refundG,food:w.food+refundF}};
    await db.set("spqr_assets",nextAssets);await saveWealth(nextWealth);await addHistory(user.id,`Sold ${biz.name}`,`${user.latinName} sold ${biz.name} in ${asset.regionName||asset.regionId}.`,`estate`);
    setAssets(nextAssets);setMsg("Property sold.");onRefresh&&onRefresh();setTimeout(()=>setMsg(""),3000);
  };
  ...
  return <div>
    ...
    <Card style={{borderLeft:`5px solid ${T.gold}`}}><STit c="Personal Wealth" sub="Your private fortune. Income is paid when the Game Master advances the season."/><div className="spqr-stat-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:"0.5rem"}}><Stat label="Personal Gold" value={`🪙 ${fmt(myW.gold)}T`} color={RES.gold.color}/><Stat label="Personal Food" value={`🌾 ${fmt(myW.food)}M`} color={RES.food.color}/><Stat label="Estate Income" value={`+${fmt(myInc.gold)}T / +${fmt(myInc.food)}M`} color={T.gre}/><Stat label="Office Salary" value={`+${fmt(salary.gold)}T / +${fmt(salary.food)}M`} color={T.blue}/><Stat label="Household Upkeep" value={`-${fmt(myW.householdGold)}T / -${fmt(myW.householdFood)}M`} color={T.rhi}/><Stat label="Properties" value={myAssets.length} color={T.blue}/></div><Row gap="0.5rem" wrap><Btn onClick={()=>donate("gold")}>Donate Gold to State</Btn><Btn v="green" onClick={()=>donate("food")}>Donate Food to State</Btn><Btn v="dark" onClick={sellFood}>Sell Personal Food</Btn></Row></Card>
    <Card><STit c="Transfers Between Senators" sub="Send private gold, food or properties to another senator."/><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:"0.55rem",alignItems:"end"}}><div><Lbl c="Recipient"/><select value={transfer.to} onChange={e=>setTransfer(t=>({...t,to:e.target.value}))} style={{width:"100%",padding:"0.48rem",border:`1px solid ${T.border}`,background:T.card}}><option value="">Choose senator...</option>{otherPlayers.map(p=><option key={p.id} value={p.id}>{p.latinName}</option>)}</select></div><div><Lbl c="Resource"/><select value={transfer.kind} onChange={e=>setTransfer(t=>({...t,kind:e.target.value}))} style={{width:"100%",padding:"0.48rem",border:`1px solid ${T.border}`,background:T.card}}><option value="gold">🪙 Gold</option><option value="food">🌾 Food</option></select></div><Inp label="Amount" type="number" value={transfer.amount} onChange={v=>setTransfer(t=>({...t,amount:v}))}/><Btn onClick={sendWealth}>Send Wealth</Btn></div></Card>
    <Card><STit c="Invest in Estates and Businesses" sub="Each province has limited slots. Senators can compete to control the best assets."/><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:"0.6rem",alignItems:"end"}}><div><Lbl c="Business Type"/><select value={buy.typeId} onChange={e=>setBuy(x=>({...x,typeId:e.target.value}))} style={{width:"100%",padding:"0.48rem",border:`1px solid ${T.border}`,background:T.card}}>{businesses.map(b=><option key={b.id} value={b.id}>{b.emoji} {b.name}</option>)}</select></div><div><Lbl c="Province"/><select value={buy.regionId} onChange={e=>setBuy(x=>({...x,regionId:e.target.value}))} style={{width:"100%",padding:"0.48rem",border:`1px solid ${T.border}`,background:T.card}}>{regions.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select></div><Btn onClick={buyAsset}>Buy Property</Btn></div><div style={{marginTop:"0.75rem",padding:"0.65rem",background:T.bg,border:`1px solid ${T.border}`}}><b>{b.emoji} {b.name}</b> in <b>{reg.name}</b><br/>Cost: 🪙 {b.costGold}T / 🌾 {b.costFood}M · Income: 🪙 +{b.incomeGold}T / 🌾 +{b.incomeFood}M · Slots: {used}/{cap}</div></Card>
    <Card><STit c="My Properties"/>{myAssets.length===0?<div style={{color:T.mut,fontStyle:"italic"}}>You own no properties yet.</div>:myAssets.map(a=>{const biz=getBiz(businesses,a.typeId);return <div key={a.id} style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"0.5rem",alignItems:"center",padding:"0.55rem",border:`1px solid ${T.border}`,marginBottom:"0.45rem",background:T.bg}}><div><b>{biz.emoji} {biz.name}</b><div style={{color:T.mut}}>{a.regionName||a.regionId} · Income: 🪙 +{biz.incomeGold}T / 🌾 +{biz.incomeFood}M</div><div style={{display:"flex",gap:"0.35rem",flexWrap:"wrap",marginTop:"0.35rem"}}><select value={propTransfer[a.id]||""} onChange={e=>setPropTransfer(x=>({...x,[a.id]:e.target.value}))} style={{padding:"0.32rem",border:`1px solid ${T.border}`,background:T.card}}><option value="">Transfer to...</option>{otherPlayers.map(p=><option key={p.id} value={p.id}>{p.latinName}</option>)}</select><Btn v="dark" sm onClick={()=>sendProperty(a)}>Transfer</Btn></div></div><Btn v="red" sm onClick={()=>sellAsset(a)}>Sell</Btn></div>})}</Card>
    <Card><STit c="My Donations"/>{donations.filter(d=>d.playerId===user.id).slice(-10).reverse().map(d=><div key={d.id} style={{padding:"0.35rem 0",borderBottom:`1px solid ${T.border}`}}>{d.kind==="gold"?"🪙":"🌾"} {fmt(d.amount)} {d.kind} donated · {d.session}</div>)}{donations.filter(d=>d.playerId===user.id).length===0&&<div style={{color:T.mut,fontStyle:"italic"}}>No donations yet.</div>}</Card>
  </div>;
}
...
function ABusinesses({D,onRefresh}){
  ...
  const regions=D.regions||DEF_REGIONS;
  const players=D.players||[];
  useEffect(()=>{(async()=>{setBusinesses((await db.get("spqr_biz"))||DEF_BUSINESSES);setAssets((await db.get("spqr_assets"))||[]);setWealth((await db.get("spqr_wealth"))||{});setDonations((await db.get("spqr_donations"))||[]);setHistory((await db.get("spqr_history"))||[]);})();},[D.players,D.regions]);
  const updBiz=(i,k,v)=>setBusinesses(bs=>bs.map((b,j)=>j===i?{...b,[k]:(k.includes("Gold")||k.includes("Food"))?Number(v):v}:b));
  const updCap=(i,regionId,v)=>setBusinesses(bs=>bs.map((b,j)=>j===i?{...b,regionCaps:{...(b.regionCaps||{}),[regionId]:Math.max(0,Number(v)||0)}}:b));
  const addBiz=()=>setBusinesses(bs=>[...bs,{id:`business_${Date.now()}`,emoji:"🏺",name:"New Business",costGold:100,costFood:0,incomeGold:20,incomeFood:0,regionCaps:{}}]);
  const save=async()=>{await db.set("spqr_biz",businesses);setMsg("Business rules saved.");onRefresh&&onRefresh();setTimeout(()=>setMsg(""),3000);};
  ...
  return <div>
    ...
    <Card><STit c="Private Wealth Control Panel" sub="Create businesses, control regional slots, track senator wealth, properties and donations."/><div className="spqr-stat-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:"0.5rem"}}><Stat label="Business Types" value={businesses.length}/><Stat label="Properties Owned" value={assets.length}/><Stat label="Gold Donated" value={`🪙 ${fmt(donationTotal("gold"))}T`} color={RES.gold.color}/><Stat label="Food Donated" value={`🌾 ${fmt(donationTotal("food"))}M`} color={RES.food.color}/></div><Row gap="0.5rem" wrap><Btn v="green" onClick={addBiz}>＋ Add Business Type</Btn><Btn onClick={save}>💾 Save Business Rules</Btn></Row></Card>
    <Card><STit c="Senator Wealth Overview" sub="Private from other players. Visible here only to the Game Master."/><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:"0.95rem"}}><thead><tr style={{background:T.bg,color:T.mut,fontFamily:"'Cinzel',serif"}}><th style={{textAlign:"left",padding:"0.45rem",border:`1px solid ${T.border}`}}>Senator</th><th style={{padding:"0.45rem",border:`1px solid ${T.border}`}}>🪙 Gold</th><th style={{padding:"0.45rem",border:`1px solid ${T.border}`}}>🌾 Food</th><th style={{padding:"0.45rem",border:`1px solid ${T.border}`}}>Estate Income</th><th style={{padding:"0.45rem",border:`1px solid ${T.border}`}}>Household</th><th style={{padding:"0.45rem",border:`1px solid ${T.border}`}}>Salary</th><th style={{padding:"0.45rem",border:`1px solid ${T.border}`}}>Properties</th></tr></thead><tbody>{players.map(p=>{const w=wealthOf(wealth,p.id);const inc=personalIncomeFor(p.id,assets,businesses);const sal=salaryFor(p.role);return <tr key={p.id}><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,fontWeight:800}}>{p.latinName}</td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,color:RES.gold.color,textAlign:"center"}}>{fmt(w.gold)}T</td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,color:RES.food.color,textAlign:"center"}}>{fmt(w.food)}M</td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,color:T.gre,textAlign:"center"}}>+{fmt(inc.gold)}T / +{fmt(inc.food)}M</td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,color:T.rhi,textAlign:"center"}}>-{fmt(w.householdGold)}T / -{fmt(w.householdFood)}M</td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,color:T.blue,textAlign:"center"}}>+{fmt(sal.gold)}T / +{fmt(sal.food)}M</td><td style={{padding:"0.45rem",border:`1px solid ${T.border}`,textAlign:"center"}}>{assets.filter(a=>a.ownerId===p.id).length}</td></tr>})}</tbody></table></div></Card>
    <Card><STit c="Business Types and Regional Limits"/>{businesses.map((b,i)=><Card key={b.id} style={{borderLeft:`4px solid ${T.gold}`}}><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:"0.5rem"}}><Inp label="Emoji" value={b.emoji} onChange={v=>updBiz(i,"emoji",v)}/><Inp label="Name" value={b.name} onChange={v=>updBiz(i,"name",v)}/><Inp label="ID" value={b.id} onChange={v=>updBiz(i,"id",v)}/><Inp label="Cost Gold" type="number" value={b.costGold} onChange={v=>updBiz(i,"costGold",v)}/><Inp label="Cost Food" type="number" value={b.costFood} onChange={v=>updBiz(i,"costFood",v)}/><Inp label="Income Gold" type="number" value={b.incomeGold} onChange={v=>updBiz(i,"incomeGold",v)}/><Inp label="Income Food" type="number" value={b.incomeFood} onChange={v=>updBiz(i,"incomeFood",v)}/></div><STit c="Slots by Province"/><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:"0.4rem"}}>{regions.map(r=><div key={r.id}><Lbl c={r.name}/><input type="number" value={(b.regionCaps||{})[r.id]||0} onChange={e=>updCap(i,r.id,e.target.value)} style={{width:"100%",padding:"0.35rem",border:`1px solid ${T.border}`}}/></div>)}</div></Card>)}</Card>
    <Card><STit c="Senator Wealth, Household Upkeep and Salary" sub="GM can adjust personal wealth and household upkeep. Salaries are calculated from current magistracy."/>{players.map(p=>{const w=wealthOf(wealth,p.id);const inc=personalIncomeFor(p.id,assets,businesses);return <div key={p.id} style={{display:"grid",gridTemplateColumns:"1.2fr repeat(7,1fr)",gap:"0.45rem",alignItems:"center",padding:"0.45rem",borderBottom:`1px solid ${T.border}`}}><b>{p.latinName}</b><input type="number" value={w.gold} onChange={e=>setPlayerWealth(p.id,"gold",e.target.value)} style={{padding:"0.35rem"}}/><input type="number" value={w.food} onChange={e=>setPlayerWealth(p.id,"food",e.target.value)} style={{padding:"0.35rem"}}/><input type="number" value={w.householdGold} onChange={e=>setPlayerWealth(p.id,"householdGold",e.target.value)} style={{padding:"0.35rem"}}/><input type="number" value={w.householdFood} onChange={e=>setPlayerWealth(p.id,"householdFood",e.target.value)} style={{padding:"0.35rem"}}/><span style={{color:T.gre}}>+{inc.gold}T/+{inc.food}M</span><span style={{color:T.blue}}>+{salaryFor(p.role).gold}T/+{salaryFor(p.role).food}M</span><span>{assets.filter(a=>a.ownerId===p.id).length} props</span></div>})}<div style={{display:"grid",gridTemplateColumns:"1.2fr repeat(7,1fr)",gap:"0.45rem",padding:"0.45rem",fontFamily:"'Cinzel',serif",color:T.mut}}><span>Senator</span><span>🪙 Gold</span><span>🌾 Food</span><span>House Gold</span><span>House Food</span><span>Estate Income</span><span>Salary</span><span>Properties</span></div></Card>
    <Card><STit c="All Properties"/>{assets.length===0?<div style={{color:T.mut,fontStyle:"italic"}}>No properties purchased yet.</div>:assets.map(a=>{const b=getBiz(businesses,a.typeId);return <div key={a.id} style={{display:"flex",justifyContent:"space-between",gap:"0.5rem",alignItems:"center",padding:"0.45rem",borderBottom:`1px solid ${T.border}`,flexWrap:"wrap"}}><span><b>{b.emoji} {b.name}</b> — {a.ownerName} — {a.regionName}</span><Row gap="0.35rem" wrap><Btn v="red" sm onClick={()=>destroyAsset(a)}>Destroy / War Loss</Btn><Btn v="ghost" sm onClick={()=>removeAsset(a.id)}>Remove</Btn></Row></div>})}</Card>
...
function PlayerApp({user:initUser,onLogout}){
  ...
  const [D,setD]=useState({players:[],game:DEF_GAME,legions:DEF_LEGIONS,regions:DEF_REGIONS,motions:[],orders:[],deadline:null,cfg:{},laws:LAWS,econ:[],election:null,elections:[],cavalry:DEF_CAVALRY,fleets:DEF_FLEETS,businesses:DEF_BUSINESSES,assets:[],wealth:{},donations:[],history:[],parties:[]});

  const refresh=useCallback(async()=>{
    const [players,game,legions,regions,motions,orders,deadline,cfg,laws,econ,election,elections,cavalry,fleets,businesses,assets,wealth,donations,history,parties]=await Promise.all([
      db.get("spqr_p"),db.get("spqr_g"),db.get("spqr_l"),db.get("spqr_r"),
      db.get("spqr_m"),db.get("spqr_o"),db.get("spqr_deadline"),db.get("spqr_cfg"),db.get("spqr_laws"),db.get("spqr_econ"),db.get("spqr_election"),db.get("spqr_elections"),db.get("spqr_cav"),db.get("spqr_f"),db.get("spqr_biz"),db.get("spqr_assets"),db.get("spqr_wealth"),db.get("spqr_donations"),db.get("spqr_history"),db.get("spqr_parties")
    ]);
...
function ABackupRestore({onRefresh}){
  const [msg,setMsg]=useState("");
  const fileRef=useRef();
  const keys=["spqr_g","spqr_l","spqr_r","spqr_p","spqr_m","spqr_o","spqr_deadline","spqr_cfg","spqr_laws","spqr_n","spqr_econ","spqr_election","spqr_elections","spqr_cav","spqr_f","spqr_biz","spqr_assets","spqr_wealth","spqr_donations","spqr_history","spqr_parties"];
...
function AResources({D,onRefresh}){
  ...
  const doAdvance=async()=>{
    ...
    for(const pl of players){
      const incP=personalIncomeFor(pl.id,assets,businesses);
      const w=wealthOf(nextWealth,pl.id);
      const sal=salaryFor(pl.role);
      const houseG=Number(w.householdGold||DEF_WEALTH.householdGold);
      const houseF=Number(w.householdFood||DEF_WEALTH.householdFood);
      nextWealth[pl.id]={...w,gold:Math.max(0,Number(w.gold||0)+incP.gold+sal.gold-houseG),food:Math.max(0,Number(w.food||0)+incP.food+sal.food-houseF)};
      if(incP.gold||incP.food||sal.gold||sal.food||houseG||houseF){
        await addHistory(pl.id,"Private Income / Salary / Household",`Estates generated ${incP.gold}T gold and ${incP.food}M food. Office salary paid ${sal.gold}T/${sal.food}M. Household upkeep cost ${houseG}T/${houseF}M for ${sLab(ng)}.`,"wealth");
      }
    }
...
function LoginScreen({onLogin,onAdmin}){
  ...
  const doReg=async()=>{
    ...
    const wealth=await db.get("spqr_wealth")||{};
    await db.set("spqr_wealth",{...wealth,[np.id]:{...DEF_WEALTH}});
