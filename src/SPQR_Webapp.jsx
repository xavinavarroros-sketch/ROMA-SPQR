...
/* ══ SENATE SEATING MAP ═══════════════════════════════════════════════════ */
function SenateMap({players,onSelectPlayer,parties=[]}){
  const [hov,setHov]=useState(null);
  const [hovPos,setHovPos]=useState({x:0,y:0});
  const W=860,H=470,SZ=62;
  const cx=W/2;
  const roleToPlayer={};
  ...
  const roleSeats={
    consul_1:{x:cx-115,y:H-82,label:"Consul I"},
    consul_2:{x:cx+115,y:H-82,label:"Consul II"},
    dictator_1:{x:cx,y:H-82,label:"Dictator"},
    magister_equitum_1:{x:cx-230,y:H-120,label:"Mag. Equitum"},
    praefectus_classis_1:{x:cx+230,y:H-120,label:"Fleet"},
    praetor_1:{x:cx-305,y:H-178,label:"Praetor I"},
    praetor_2:{x:cx+305,y:H-178,label:"Praetor II"},
    quaestor_1:{x:cx-370,y:H-246,label:"Quaestor I"},
    quaestor_2:{x:cx+370,y:H-246,label:"Quaestor II"},
    aedile_1:{x:cx,y:H-190,label:"Aedile"},
    tribune_1:{x:cx-150,y:H-178,label:"Tribune I"},
    tribune_2:{x:cx+150,y:H-178,label:"Tribune II"},
  };

  const arcSeats=[];
  const rows=[
    {radius:355,count:18,start:204,end:336},
    {radius:292,count:16,start:208,end:332},
    {radius:230,count:14,start:214,end:326},
    {radius:170,count:10,start:225,end:315},
  ];
  rows.forEach((row,rowIdx)=>{
    for(let i=0;i<row.count;i++){
      ...
      arcSeats.push({
        ...
        y:H-42+row.radius*Math.sin(rad),
        row:rowIdx
      });
    }
  });
  ...
  function Seat({pl,posInfo,x,y,empty=false,regular=false}){
    ...
    return <div
      ...
      onClick={()=>{if(pl&&onSelectPlayer)onSelectPlayer(pl);}}
      ...
      {empty&&<div style={{width:"100%",height:"100%",opacity:0.12,background:`repeating-linear-gradient(45deg,${T.fnt},${T.fnt} 1px,transparent 1px,transparent 8px)`}}/>}
    </div>;
  }

  return(
    <div className="spqr-senate-scroll" style={{position:"relative",userSelect:"none",overflowX:"auto",paddingBottom:"0.5rem"}}>
      <div style={{position:"relative",width:W,height:H,margin:"0 auto",minWidth:W,background:`radial-gradient(ellipse at 50% 94%, ${T.surf} 0%, ${T.bg} 42%, ${T.card} 100%)`,border:`2px solid ${T.bhi}`,borderRadius:"18px 18px 80px 80px",overflow:"hidden"}}>
        {/* Half-moon bench guides */}
        {[360,298,236,174].map((r,i)=><div key={r} style={{position:"absolute",left:cx-r,top:H-42-r,width:r*2,height:r*2,border:`1px solid ${i===0?T.bhi:T.border}`,borderBottom:"none",borderRadius:"50%",opacity:i===0?0.55:0.35,pointerEvents:"none"}}/>)}
        <div style={{position:"absolute",left:cx-115,top:H-190,width:230,height:118,border:`1px solid ${T.bhi}`,background:"rgba(255,255,255,0.45)",borderRadius:"14px",display:"flex",alignItems:"center",justifyContent:"center",textAlign:"center",fontFamily:"'Cinzel',serif",letterSpacing:"0.14em",color:T.ghi,fontSize:"0.72rem",lineHeight:1.4}}>Senate Floor<br/>Magistrate Dais</div>
        ...
        {arcSeats.map((pt,idx)=>{
          const pl=regulars[idx]||null;
          return <Seat key={pt.key} pl={pl} regular x={pt.x} y={pt.y} empty={!pl}/>;
        })}
      </div>
...
function SenatePanel({players,D}){
  ...
  return(
    <div>
      ...
      <Card>
        <STit c="Senate Seating" sub="Click a senator to open his profile. Party members are outlined/underlined with their party color."/>
        <SenateMap players={players} parties={parties} onSelectPlayer={setSelected}/>
...
function PlayerApp({user:initUser,onLogout}){
  ...
  const [D,setD]=useState({players:[],game:DEF_GAME,legions:DEF_LEGIONS,regions:DEF_REGIONS,motions:[],orders:[],deadline:null,cfg:{},laws:LAWS,econ:[],election:null,elections:[],cavalry:DEF_CAVALRY,fleets:DEF_FLEETS,businesses:DEF_BUSINESSES,assets:[],wealth:{},donations:[],history:[],parties:[],wealthlog:[],cemetery:[]});

  const refresh=useCallback(async()=>{
    const [players,game,legions,regions,motions,orders,deadline,cfg,laws,econ,election,elections,cavalry,fleets,businesses,assets,wealth,donations,history,parties,wealthlog,cemetery]=await Promise.all([
      db.get("spqr_p"),db.get("spqr_g"),db.get("spqr_l"),db.get("spqr_r"),
      db.get("spqr_m"),db.get("spqr_o"),db.get("spqr_deadline"),db.get("spqr_cfg"),db.get("spqr_laws"),db.get("spqr_econ"),db.get("spqr_election"),db.get("spqr_elections"),db.get("spqr_cav"),db.get("spqr_f"),db.get("spqr_biz"),db.get("spqr_assets"),db.get("spqr_wealth"),db.get("spqr_donations"),db.get("spqr_history"),db.get("spqr_parties"),db.get("spqr_wealthlog"),db.get("spqr_cemetery")
    ]);
  ...
  const TABS=[
    {sep:"🏛️ GOVERNMENT",tone:"gov"},
    {k:"senate",l:"Senate",tone:"gov"},
    {k:"voting",l:`Voting${votingCount>0?` (${votingCount})`:""}`,tone:"gov"},
    {k:"orders",l:"Orders",tone:"gov"},
    {k:"resources",l:"Resources",tone:"gov"},
    {k:"legions",l:"Legions",tone:"gov"},
    {k:"magistrates",l:"Magistrates",tone:"gov"},
    {k:"elections",l:"Elections",tone:"gov"},
    ...(pos?[{k:"office",l:`${pos.abbr}`,tone:"office"}]:[]),
    {sep:"👤 PERSONAL",tone:"personal"},
    {k:"wealth",l:"Personal Wealth",tone:"personal"},
    {k:"parties",l:"Parties",tone:"personal"},
    {k:"character",l:"Character",tone:"personal"},
    {sep:"📜 RECORDS",tone:"records"},
    {k:"cemetery",l:"Cemetery",tone:"records"},
    {k:"laws",l:"Laws",tone:"records"},
    {k:"map",l:"Map",tone:"records"},
  ];
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg}}>
      ...
      <div className="spqr-tabs" style={{display:"flex",borderBottom:`1px solid ${T.border}`,background:T.surf,overflowX:"auto",position:"sticky",top:"45px",zIndex:99}}>
        {TABS.map((it,i)=>it.sep?(
          <div key={`sep-${it.sep}-${i}`} style={{padding:"0.58rem 0.55rem",background:toneBg(it.tone),color:toneColor(it.tone),borderRight:`1px solid ${T.border}`,fontFamily:"'Cinzel',serif",fontSize:"0.66rem",fontWeight:900,letterSpacing:"0.16em",borderTop:`3px solid ${toneColor(it.tone)}`,whiteSpace:"nowrap",flexShrink:0,display:"flex",alignItems:"center"}}>{it.sep}</div>
        ):(
          <button key={it.k} onClick={()=>setTab(it.k)} style={{padding:"0.55rem 0.9rem",background:tab===it.k?toneBg(it.tone):"transparent",color:tab===it.k?toneColor(it.tone):T.mut,border:"none",borderBottom:tab===it.k?`2px solid ${toneColor(it.tone)}`:"2px solid transparent",fontFamily:"'Cinzel',serif",fontSize:"0.9rem",letterSpacing:"0.1em",whiteSpace:"nowrap",flexShrink:0}}>
            {it.l}
          </button>
        ))}
      </div>
...
function ASenators({D,onRefresh}){
  ...
  return(
    <div>
      ...
      <Card>
        <STit c="Senate Seating" sub="GM view: click any occupied seat to open the senator profile. Party colors appear on seats."/>
...
function ABackupRestore({onRefresh}){
  const [msg,setMsg]=useState("");
  const fileRef=useRef();
  const keys=["spqr_g","spqr_l","spqr_r","spqr_p","spqr_m","spqr_o","spqr_deadline","spqr_cfg","spqr_laws","spqr_n","spqr_econ","spqr_election","spqr_elections","spqr_cav","spqr_f","spqr_biz","spqr_assets","spqr_wealth","spqr_donations","spqr_history","spqr_parties","spqr_cemetery"];
...
function AdminApp({onLogout}){
  ...
  const TABS=[
    {sep:"🏛️ GOVERNMENT",tone:"gov"},
    {k:"overview",l:"Overview",tone:"gov"},
    {k:"senators",l:`Senators (${(D.players||[]).length})`,tone:"gov"},
    {k:"legions",l:"Legions",tone:"gov"},
    {k:"resources",l:"Resources & Regions",tone:"gov"},
    {k:"magistrates",l:"Magistrates",tone:"gov"},
    {k:"elections",l:"Elections",tone:"gov"},
    {k:"motions",l:`Motions${pendM?` (${pendM})`:""}`,tone:"gov"},
    {k:"orders",l:`Orders${newO?` (${newO})`:""}`,tone:"gov"},
    {sep:"👤 PERSONAL / POLITICS",tone:"personal"},
    {k:"businesses",l:"Private Wealth",tone:"personal"},
    {k:"parties",l:"Parties",tone:"personal"},
    {sep:"📜 RECORDS / SETUP",tone:"records"},
    {k:"registry",l:"Registry",tone:"records"},
    {k:"cemetery",l:`Cemetery (${(D.cemetery||[]).length})`,tone:"records"},
    {k:"laws",l:"Laws",tone:"records"},
    {k:"setup",l:"Setup",tone:"records"},
  ];
